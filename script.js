...
const EMBLEM_REVEAL_DELAY = 620;
const WORDMARK_REVEAL_DELAY = 3720;
const SUBMARK_REVEAL_DELAY = 3980;
const FORM_REVEAL_DELAY = 4380;
const EMBLEM_FADE_DELAY = 2480;
const THRESHOLD_REVEAL_DELAY = 3300;
...
function syncActiveNav(id) {
  ...
  const activeMobileIndex = mobileNavItems.findIndex(item => item.dataset.target === id);
  mobileNavItems.forEach((item, index) => {
    const isActive = item.dataset.target === id;
    const isAdjacent = activeMobileIndex !== -1 && Math.abs(index - activeMobileIndex) === 1;
